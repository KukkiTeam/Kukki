...
public class UserModel {
    ...
    public boolean insertUser(int id, String name, String email, String api_key, String avatar_url, int total_receipt, int total_followed){
        boolean check=true;
        ...
        content.put("total_receipt", total_receipt);
        content.put("total_followed", total_followed);
    ...
    public String getTotalReceipt(){
        String sql="select total_receipt from User";
        String total_receipt="";
        Cursor c=mdatabase.rawQuery(sql,null);
        if(c.getCount()>0){
            c.moveToFirst();
            do{
                total_receipt=c.getString(0);
            }while (c.moveToNext());
        }
        c.close();
        return total_receipt;
    }
    public String getTotalFollowed(){
        String sql="select total_followed from User";
        String total_followed="";
        Cursor c=mdatabase.rawQuery(sql,null);
        if(c.getCount()>0){
            c.moveToFirst();
            do{
                total_followed=c.getString(0);
            }while (c.moveToNext());
        }
        c.close();
